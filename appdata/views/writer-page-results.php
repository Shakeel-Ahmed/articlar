<div class="container">
    <div class="row mt-3 mb-5">
        ...
        <div class="col-md-6 search-box-block mt-3 mb-3 mt-md-0 mb-md-0">
            <form action="javascript:artSearch('<?php echo base_url(); ?>author/admin/articles/');" class="cross-center">
                <input type="text" id="art-search-field" class="art-search-box space-left" placeholder="Search" style="margin-left: auto;">
                <i class="art-icon hover" onclick="artSearch('<?php echo base_url(); ?>author/admin/articles/');">search</i>
            </form>
        </div>
    </div>
    ...
    foreach($rows as $row)
    {
        ...
        switch($row['status'])
        {
            ...
            case 4 : {$status_color = 'publish ';    $delete_status = 'locked';         $icon='radio_button_checked';} break;
        ...
        <div class="row result-row page-<?php echo $row['page-id']; ?>">
            <div class="col-md-9">
                <div class="row cross-center">
                    <div class="col-md-4 center">
                        <img onclick="go('<?php echo base_url(); ?>author/admin/edit-article/<?php echo slug($row['title']).'/'.$row['page-id']; ?>');"
                             id="art-btn-<?php echo $row['page-id']; ?>" class="article-search-result-image hover border-color-<?php echo $status_color ;?>"
                             src="<?php echo $image; ?>" style="width: 200px;height:200px;">
                    </div>
                    <div class="col-md-8">
                        <div class="mt-4 mb-4 mt-md-0 mb-md-0 text-center text-md-left">
                            <a class="hover" href="<?php echo base_url(); ?>author/admin/edit-article/<?php echo slug($row['title']).'/'.$row['page-id'];?>">
            ...
            <div class="col-md-3 center cross-center mb-4 mb-md-0">
                ...
                <i class="art-icon ml-3 hover" onclick="go('<?php echo base_url(); ?>author/admin/preview-article/<?php echo slug($row['title']).'/'.$row['page-id']; ?>');">launch</i>
                <i class="art-icon ml-3 hover" onclick="deleteArticle('<?php echo $delete_status; ?>','<?php echo base_url(); ?>author/admin/delete-article/',false);">delete</i>
                <i class="art-icon ml-3 text-color-inprogress hover" onclick="setPublishStatus(<?php echo $row['status']; ?>,'1','<?php echo $row['page-id']; ?>','<?php echo base_url() ;?>author/admin/set-page-status');">radio_button_checked</i>
                <i class="art-icon ml-3 text-color-review hover" onclick="setPublishStatus(<?php echo $row['status']; ?>,'3','<?php echo $row['page-id']; ?>','<?php echo base_url() ;?>author/admin/set-page-status');">radio_button_checked</i>
            </div>
        </div>
