...
class Common_model extends CI_Model {
    ...
    public function generate_html($page_id)
    {
        ...
        $values['body'] = str_replace(['[art-404]','[art-noresult]','[art-results]','<p>[deco-border-end]</p>','[column-end]','<p>[container-end]</p>','[add-columns-here]','<p>[jumbotron-end]</p>'],'',$values['body']);
        if(strpos($values['body'],'[art-blog]')!==false)
        {
            ...
            {
                $noresult = $this->db->query("SELECT `body` FROM `{$this->config->item('DBTPage')}` WHERE `keywords` = '[art-noresults]'")->row_array();
