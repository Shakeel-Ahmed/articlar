	...
	class Common_model extends CI_Model {
        ...
        public function search_model($search_data)
        {
             ...
             elseif(is_array($search_data['search']) and $search_data['search-type'] == 'fulltext')
            {
                ...
                {
                   if(!$search_value or $search_value=='datasearch') continue;
                    $search_string[] =  "MATCH (`$db_column`) AGAINST ('$search_value' {$search_data['fulltext-mode']})";
                }
            }
            if(!empty($search_string)) $searchSQL = 'WHERE '.implode(' OR ',$search_string);
            else $searchSQL = null;

            if(is_array($search_data['restrict']) and $search_data['restrict'] != null)
            {
                foreach($search_data['restrict'] as $id_column => $values)
                {
                    $id_sql[] = "`$id_column` ".$values[key($values)]." '".key($values)."'";
                }
        ...
        public function generate_html($page_id)
        {
            ...
            if($page_id == 'results')
            {
                $values = $this->db->get_where($this->config->item('DBTPage'),['keywords'=>'[art-results]'])->row_array();
                if(!$values) $values =
                    [
                        'keywords'    => 'search results',
                        'description' => 'User search results display',
                        'title'       => 'Search Results',
                        'header'      => null,
                        'footer'      => null,
                        'body'        => '<div class="container art-header-spacing">[art-blog]</div>',
                        'status'      => 4,
                    ];
            }
            else
            ...
            $values['body'] = str_replace(['[art-noresult]','[art-results]','<p>[deco-border-end]</p>','<p>[container-end]</p>','[add-columns-here]','<p>[jumbotron-end]</p>'],'',$values['body']);
            ...
            if(strpos($values['body'],'[art-blog]')!==false)
            {
                ...
                (
                    ...
                    'restrict' => ['title'=>['404'=>'!='],'title'=>['noresult'=>'!='],'keywords'=>['[art-results]'=>'!=']],
                ...
                if($data['results']!==0)
                {
                    ...
                    $pagination = array
                    (
                        'url'       => base_url().$segments.$page_id.'/'.$search,
                ...
                {
                    $noresult = $this->db->query("SELECT `body` FROM `{$this->config->item('DBTPage')}` WHERE `keywords` = '[art-noresult]'")->row_array();
                    if($noresult) $articles = $noresult['body'];
                    else $articles = '<div style="text-align:center;"><h3>Sorry! No Articles Found</h3><hr/><p style="text-align: center;">Sorry! Your search did not fetch any result. Please try again with different search criteria.<h4>Thank You!</h4></p></div>';
                }
            ...
            $search_javascript = '<script>function artSearch(e){var a=document.getElementById("art-search-field").value.toString().toLowerCase().replace(/\s+/g,"-").replace(/[^\w\-]+/g,"").replace(/\-\-+/g,"-").replace(/^-+/,"").replace(/-+$/,"");if(""==a)return!1;window.location.href=e+a}</script></head>';
        ...
        public function page_delete_model($page_id)
        {
            ...
            else return '{"status":"fail","title":"Error","message":"Unable to complete the request due to database error. Please try again."}';
        }
        public function page_edit_model($page_id,$mod=null)
        {
            if(ptrig('trigger','gobaby'))
            {
                ...
                if($this->db->update($this->config->item('DBTPage')))
                {
                    ...
                    (
                        'status'    => true,
                        'title'     => 'Page Updated',
                        'message'   => 'Page has been successfully updated and all changes are made.',
                        'action'	=> null
                    );
                }
            }
            ...
            if($mod == 'author' and $page['status'] > 2) return
                [
                    ...
                    'message'   => 'Homepage, published or under review articles can not be edited by author. Only Editor can edit or unlock this article and make it editable for you by changing article status to unpublish or back to work in progress.',
                    'action'	=> null
                ];
        ...
        public function page_preview_model($page_id,$mod)
        {
            ...
            <i class="art-icon hover" onclick="goBack();">arrow_back</i>
            </div>',$page['body']);
        ...
        public function author_edit_model($author_id)
        {
                ...
                if($login_chk)
                {
                    return
                        [
                            ...
                            'message' => 'Email you have provided already exist with another account. Please use another email, two authors can not use single email address.',
            ...
            else $data['picture'] = base_url().'images/nopicture.svg';
        ...
        public function filebrowser_model($author_id)
        {
            if(ptrig('picture'))
            {
                if(unlink($_POST['picture']) and unlink(str_replace('tmb_','',$_POST['picture']))) die('true');
                else die('false');
            }
            if(ptrig('trigger','gobaby'))
            {
                if(is_uploaded_file($_FILES['gallery']['tmp_name'][0]))
                {
                    ...
                    for($i=0;$i<=$uploads;$i++)
                    {
                        ...
                        (
                            'field'         => $formfield,
                            'upload_path'   => $this->config->item('uploadir').'authors/'.$author_id,
                            'file_name'     => ranum(16),
                            'allowed_types'     => array('jpeg','jpg','png','gif','svg'),
                            'file_ext_tolower'  => TRUE,
                            'thumb_width'       => 360,
                        ...
                        $this->common_model->image_uploader($upload_parameters);
                    }
                }
            }
        ...
        public function image_replace_model($author_id,$image)
        {
            ...
            if(ptrig('trigger','goBabyReplaceImage'))
            {
                ...
                $this->image_uploader($upload_parameters);
        ...
        public function image_uploader($upload)
		{
			...
			if($file['file_name'] and ($upload['thumb_width'] or $upload['image_width']))
			{
				...
				{
					if (isset($upload['tiny_width'])) resizeGD($src_image, $tiny_image, $upload['tiny_width'], $upload['tiny_quality']);
					if (isset($upload['thumb_width'])) resizeGD($src_image, $tmb_image, $upload['thumb_width'], $upload['thumb_quality']);
		...
		public function sendmail($data)
        {
            ...
            if(!$status)
            {
                ...
                [
                    ...
                    'message'   => 'Send email failed due to mailserver or config error. Please try again.',
                    'action'    => null
                ];
            }
            else return
